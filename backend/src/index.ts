...
app.post('/api/analyze', async (c) => {
	try {
      ...
      **A. install_time_estimate (預估工時，單位：分鐘，Number)**
        - 基礎時間：30 分鐘
        - 加時規則 (累加)：
          - 若包含「滾筒洗衣機」或「洗脫烘」：+25 分鐘
          - 若包含「冷氣」或「空調」或「分離式」：+150 分鐘
          - 若包含「回收」或「舊機」：+10 分鐘
          - 若地址包含「3F」「4F」「5F」或更高樓層且無「電梯」字樣：+10 分鐘
      ...
      **D. distance_check (較遠路程判定 - 優先級最高)**
        - **規則 1 (強制命中)**：若地址包含以下任一區域名稱，**必須** 加入 'remote_area' tag，不需計算時間：
          - 楠梓、仁武、左營、鼓山、大社、橋頭、岡山、路竹、湖內、鳥松
          - 林園、大樹、大寮
        - **規則 2 (時間估算)**：若未包含上述區域，但預估從「高雄市鳳山區林森路291號」開車超過 **20 分鐘**，也請加入 'remote_area'。
